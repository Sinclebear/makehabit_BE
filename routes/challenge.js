...
router.get("/search",async(req,res)=>{
    ...
    const existChallenges = await Challenge.find({ title: { $regex: `${title}` } },{_id:1,participants:1,thumbnail:1,title:1,startAt:1}).lean()
...
router.get("/category/:categoryId", async(req,res)=>{
    ...
    const existChallenges = await Challenge.find({ category: categoryId },{_id:1,participants:1,thumbnail:1,title:1,startAt:1}).limit(length).lean()
...
router.post("/challenges", async(req,res)=>{  // 로그인 기능 완료 되면 authMiddleware 통과시키기!!
    try {
