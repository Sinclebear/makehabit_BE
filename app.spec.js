...
describe('로그인 체크', () => {
    beforeEach(() => {
        const token =
            'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJuaWNrbmFtZSI6ImRid25ndXMiLCJlbWFpbCI6ImJnZzAxNTc4QG5hdmVyLmNvbSIsImlhdCI6MTUxNjIzOTAyMn0.uS8OyE60TKG-jieFnkpn6pppZ8iV847b6nM7pQoL9bo';
        authMiddleware = jest.fn();
    });
    test('조건에 맞는 입력 시 토큰에 해당하는 사용자의 이메일과 닉네임 반환', async () => {
        const res = await supertest(app)
        ...
        expect(res.body).toEqual({ email: 'bgg01578@naver.com', nickname: 'dbwngus' });
    });
});
afterAll(() => {
